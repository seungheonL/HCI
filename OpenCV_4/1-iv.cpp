...
int main()
{
	...
	result = cedge.GS_canny_edge_Canny(jenny_gray); // canny edge 적용

	imshow("result", result); // 이미지 출력
	imwrite("result/jenny_canny.png", result); // 이미지 저장

	waitKey(); // Output 이미지를 관찰하기 위해 별도의 입력이 있을 때까지 기다린다.

	result = cedge.GS_canny_edge_Canny(rice_gray); // canny edge 적용

	imshow("result", result); // 이미지 출력
	imwrite("result/rice_canny.png", result); // 이미지 저장
