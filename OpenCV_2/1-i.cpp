...
int main()
{
	...
	result = cpixel.GS_imhist(lenna_gray); // lenna_gray에 대한 히스토그램을 만들어준다.

	imshow("result", result); // Output 이미지 출력
	imwrite("result/lenna_histogram.png", result); // Output 이미지를 내 프로젝트에 별도로 저장
	...
	result = cpixel.GS_imhist(stuff_color_1_gray); // stuff_color_1_gray에 대한 히스토그램을 만들어준다.

	imshow("result", result); // Output 이미지 출력
	imwrite("result/stuff_color_1_gray_histogram.png", result); // Output 이미지를 내 프로젝트에 별도로 저장
